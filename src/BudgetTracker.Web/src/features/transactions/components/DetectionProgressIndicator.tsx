...
export function DetectionProgressIndicator({
  ...
  const getPhaseDescription = (): string => {
    switch (currentPhase) {
      case 'uploading':  return `Uploading ${isImage ? 'bank statement image' : 'CSV file'}...`;
      case 'detecting':  return isImage ? 'Preparing image for analysis...' : 'Detecting CSV structure...';
      case 'parsing':    return 'Parsing CSV data...';
      case 'extracting': return 'Extracting transactions from image using AI...';
      case 'enhancing':  return 'Enhancing transaction descriptions with AI...';
  ...
  return (
    <div className="mt-4">
      ...
      <p className="mt-1 text-xs text-gray-400 truncate">{fileName}</p>
    </div>
  );
}

export type { DetectionProgressIndicatorProps };
