...
export function RecommendationsCard({ recommendations }: RecommendationsCardProps) {
  const getIcon = (type: ProactiveRecommendation['type']) => {
    switch (type) {
      case 'SpendingAlert': return <AlertTriangleIcon />;
      case 'SavingsOpportunity': return <DollarSignIcon />;
      case 'TrendInsight': return <TrendingDownIcon />;
      case 'CategoryOptimization': return <TagIcon />;
  ...
  const formatType = (type: string) => type.replace(/([A-Z])/g, ' $1').trim();
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm opacity-90 leading-relaxed">{recommendation.description}</p>
